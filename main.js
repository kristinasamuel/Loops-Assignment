...
var guessNumberStatus = false;
var userGuess = [4, 3, 5, 7, 9, 8, 2, 1, 6, 10]; // an array to store a series of predefined guesses
var i = 0;
// use loop 
while (i < userGuess.length) {
    var Currentguess = userGuess[i];
...
for (var items in myObject) {
    console.log("".concat(items, ":").concat(myObject[items]));
}
...
for (var i_3 = 0; i_3 < myArray.length; i_3++) {
    console.log("indexNo ".concat(i_3, ":  element ").concat(myArray[i_3]));
}
